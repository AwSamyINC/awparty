...
## Безопасность: НИЧЕГО не скачивать на компьютер пользователя

**СТРОГИЙ ЗАПРЕТ.** Тебе **запрещено** что-либо скачивать или устанавливать на
компьютер пользователя. Сюда входит (не ограничиваясь): `npm install` / `npm ci` /
`npm i`, `npx <пакет>` с автозагрузкой, `npx playwright install`, `pip install`,
`curl` / `wget` / `Invoke-WebRequest` для скачивания файлов, загрузка бинарников,
браузеров, моделей, docker-образов и любых других внешних артефактов.

Если скачивание **действительно необходимо** для задачи — **не делай его молча**.
Останови работу и **трижды задай один и тот же вопрос**: что именно и зачем нужно
скачать. Продолжить можно **только** после того, как пользователь на **каждый** из
трёх вопросов ответит **дословно**: **«Да, я разрешаю скачать»**. Любой другой
ответ — или меньше трёх подтверждений — означает запрет: действуй без скачивания
(или останови задачу, если без этого никак).

*(Strict ban on downloading/installing anything onto the user's machine. If truly
necessary, ask the SAME question three times; proceed only after the user replies
verbatim «Да, я разрешаю скачать» to all three.)*
