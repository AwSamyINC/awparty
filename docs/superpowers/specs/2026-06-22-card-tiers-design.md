...
Common: **урон (1) и HP+лечение (4) — без потолка** (бесконечны, как сейчас);
**скорострельность (0), скорость (2), магнит (3) — потолок 10** (после 10 карта
выбывает из пула). У скорости/магнита прежний стат-кап (400/600) достигается
раньше 10 уровней — карта продолжает выпадать до 10, но сверх стат-капа эффект
не растёт (поведение не меняем). Счётчик уровня выводится только на конечных
многоуровневых картах (см. «Визуал карт»); у бесконечных (1, 4) счётчика нет.
...
**Исчерпание пула.** Урон (1) и HP (4) бесконечны → как минимум 2 карты всегда
доступны, случай «0 карт» недостижим. Но дроп может ужаться до 2 карт, если все
конечные карты (0,2,3 + все Rare/Legendary) уже на максе — тогда остаются только
урон и HP. Экран показывает столько карт, сколько доступно. На случай будущих
правок (если обе бесконечные карты тоже когда-нибудь ограничат) оставляем
страховку: 0 доступных → пропуск экрана + полный хил (`p.hp = p.maxHp`) и
возврат в `PLAYING`, чтобы исключить софт-лок.
...
- Веса тиров: **64 / 28 / 8**.
- Common: урон (1) и HP (4) — **бесконечны**; скорострельность (0), скорость (2),
  магнит (3) — потолок **10**. После макса карта **убирается из пула**.
- Исчерпание пула: <3 доступных → меньше карт на экране (минимум 2 — урон+HP);
  0 недостижимо при текущих правилах (страховка с хилом оставлена).
