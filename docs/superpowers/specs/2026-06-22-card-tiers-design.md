...
|----|-------|-----|-----------|-----|
| 0 | Скорострельность | Common | ∞ | есть (`icon_fire`) |
| 1 | Урон | Common | ∞ | есть (`icon_dmg`) |
| 2 | Скорость | Common | ∞ (стат-кап 400) | есть (`icon_speed`) |
| 3 | Магнит | Common | ∞ (стат-кап 600) | есть (`icon_magnet`) |
...
Common — без потолка (всегда доступны, как сейчас; у скорости/магнита остаётся
прежний стат-кап, карта при этом продолжает выпадать — поведение не меняем).
...
const CARD_MAX_LEVEL = [
  Infinity, Infinity, Infinity, Infinity, Infinity, // commons
  1,  // 5 blademail
  1,  // 6 pierce
  3,  // 7 shield
  1,  // 8 crit
...
для каждого из 3 слотов:
  ...
  пока cand пуст и tier > COMMON: tier--; cand = poolByTier(tier) минус ids
  (COMMON неисчерпаем → cand не будет пустым)
...
- Веса тиров: **64 / 28 / 8**.
- Макс-карты **убираются из пула** (как нынешние легендарки).
