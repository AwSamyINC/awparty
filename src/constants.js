...
const C = {
    ...
    // Отступ спавна винила от стен. Винил не притягивается магнитом (см. Vinyl.update),
    // а центр игрока клэмпится к [halfW..arenaW-halfW] (~60px). У самой стены радиус
    // подбора (50px) не дотягивается — поэтому спавним заведомо в досягаемой зоне.
    VINYL_SPAWN_MARGIN: 120,
...
// Пыль из-под ног при ходьбе — частиц в секунду. Привязка к dt, чтобы на высокой
// частоте кадров не плодилось кратно больше (12/с == прежние 20%/кадр при 60 fps).
C.DUST_PER_SEC = 12;
