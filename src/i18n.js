
const I18N = {
    en: {
        menu_play: 'Play',
        menu_records: 'Records',
        menu_achievements: 'Achievements',
        menu_settings: 'Settings',

        ach_title: 'ACHIEVEMENTS',
        ach_back: 'BACK',
        ach_unlocked_run: 'Achievements unlocked!',
        ach_done: 'Unlocked',
        ach_of: 'of',
        ach_sort_default: 'Default',
        ach_sort_name: 'By name',
        ach: {
            first_run:      { title: 'First Steps',        desc: 'Finish your first run' },
            rich_run:       { title: 'Coin Hoarder',       desc: 'Collect 120 coins in one run' },
            ch1_clear:      { title: 'Chapter 1 Cleared',  desc: 'Defeat the final boss of Chapter 1' },
            ch2_clear:      { title: 'Chapter 2 Cleared',  desc: 'Defeat the final boss of Chapter 2' },
            ch3_clear:      { title: 'Gauntlet Cleared',   desc: 'Beat the boss trio of Chapter 3' },
            score_10k:      { title: 'Sharpshooter',       desc: 'Score 10,000 in one run' },
            score_18k:      { title: 'Virtuoso',           desc: 'Score 18,000 in one run' },
            survivor_10:    { title: 'Survivor',           desc: 'Survive 10 minutes in one run' },
            survivor_15:    { title: 'Last One Standing',  desc: 'Survive 15 minutes in one run' },
            massacre_500:   { title: 'Massacre',           desc: 'Kill 500 enemies in one run' },
            massacre_1000:  { title: 'Unstoppable',        desc: 'Kill 1000 enemies in one run' },
            untouchable:    { title: 'Untouchable',        desc: 'Clear a chapter taking 8 hits or fewer' },
            hardcore_clear: { title: 'Hardcore Hero',      desc: 'Clear any chapter in Hardcore mode' },
            slayer_1:       { title: 'Slayer I',           desc: 'Kill 1,000 enemies total' },
            slayer_2:       { title: 'Slayer II',          desc: 'Kill 5,000 enemies total' },
            slayer_3:       { title: 'Slayer III',         desc: 'Kill 10,000 enemies total' },
            slayer_4:       { title: 'Slayer IV',          desc: 'Kill 25,000 enemies total' },
            slayer_5:       { title: 'Slayer V',           desc: 'Kill 50,000 enemies total' },
            slayer_6:       { title: 'Slayer VI',          desc: 'Kill 100,000 enemies total' },
            veteran_1:      { title: 'Veteran I',          desc: 'Play 25 runs' },
            veteran_2:      { title: 'Veteran II',         desc: 'Play 100 runs' },
        },

        lobby_hub: 'HUB',
        lobby_start: 'START RUN',
        lobby_shop: 'UPGRADES & SHOP',
        lobby_back: 'Back to Menu',

        chapter_select_title: 'SELECT CHAPTER',
        chapter_label: 'CHAPTER',
        chapter_play: 'PLAY',
        chapter_locked: 'CLOSED',
        chapter_hint_back: 'BACK TO HUB',

        set_title: 'SETTINGS',
        set_grp_game: 'GAMEPLAY',
        set_grp_audio: 'AUDIO',
        set_grp_display: 'DISPLAY',
        set_grp_account: 'ACCOUNT',
        set_cloud_open: 'open',
        set_hardcore: 'Hardcore Mode',
        set_window: 'Window Mode',
        set_sound: 'Sound',
        set_effects: 'Effects',
        set_language: 'Language',
        set_rename: 'Rename Player',
        on: 'ON',
        off: 'OFF',
        win_full: 'FULLSCREEN',
        win_windowed: 'WINDOWED',
        not_set: '(not set)',
        back: 'Back',
        lang_en: 'English',
        lang_ru: 'Русский',
        cheat_gave: 'Gave 500 coins',
        cheat_noname: 'No player name yet — set one by scoring first',

        lb_title: 'RECORDS',
        lb_hardcore: 'HARDCORE',
        lb_normal: 'NORMAL',
        lb_col_num: '#',
        lb_col_name: 'NAME',
        lb_col_score: 'SCORE',
        lb_col_time: 'TIME',
        lb_col_date: 'DATE',
        lb_hint_switch: '<  /  >   -   Normal / Hardcore',
        lb_chapter: 'CHAPTER',
        lb_your_place: 'Your place',
        lb_hint_chapter: '◀ ▶  Chapter',
        lb_hint_mode: '▲ ▼  Mode',
        lb_hint_sort: 'TAB / click  -  Sort (time / score)',
        lb_hint_back: 'BACK',

        pause_title: 'PAUSED',
        pause_resume: 'Resume',
        pause_restart: 'Restart',
        pause_quit: 'Quit to Hub',

        gameover: "GAME OVER\nPress 'R' to Restart\nPress 'Q' for Hub",
        gameover_records: 'L  -  View Records',

        name_new_record: 'NEW RECORD!',
        name_score: 'Score:',
        name_time: 'Time:',
        name_enter: 'Enter your name:',
        name_hint: 'ENTER  -  Confirm        BACKSPACE  -  Erase',
        err_enter_name: 'Enter a name',
        err_name_taken: 'Name is taken',

        rename_title: 'RENAME PLAYER',
        rename_current: 'Current name:',
        rename_new: 'New name:',
        rename_saving: 'Saving...',
        rename_merge_note: 'Merges leaderboard records, keeps your best time per mode.',
        rename_hint: 'ENTER  -  Confirm        ESC  -  Cancel',
        err_server: 'Server error, try again',

        levelup: 'LEVEL UP!',
        first_card: 'STARTING CARD',
        ability_choose: 'CHOOSE AN ABILITY',
        cooldown: 'Cooldown',
        ability_desc_0: 'Become invulnerable\nfor 2 seconds.',
        ability_desc_1: 'Slam the ground to\ndamage and knock back\nnearby enemies.',
        ability_desc_2: 'Unleash 12 vinyl discs\nin all directions.',
        ability_desc_3: 'Fire a piercing laser\nbeam toward the cursor.',
        ability_desc_4: 'Hurl a skull that ricochets\nthrough 5 enemies,\n+20% damage per bounce.',
        ability_desc_5: 'Emit a sonic wave that\nknocks back and damages\nall nearby enemies.',
        ability_desc_6: 'Hurl a charge that bursts\ninto 14 shards flying\nin all directions.',

        upgrade_toasts: ['UPGRADE: Fire Rate +', 'UPGRADE: Damage +', 'UPGRADE: Speed +', 'UPGRADE: Magnet +', 'UPGRADE: Max HP +1', 'UNLOCKED: Blademail', 'UNLOCKED: Pierce Shot', 'UPGRADE: Energy Shield', 'UPGRADE: Crit +10%', 'UPGRADE: Orb', 'UPGRADE: Double Tap'],

        upgrade_titles: ['FAST TRIGGER', 'BRUTE FORCE', 'RUNNER', 'MAGNETISM', 'HEALTH UP', 'BLADEMAIL', 'PIERCE SHOT', 'ENERGY SHIELD', 'CRIT BOOST', 'ORB', 'DOUBLE TAP'],
        upgrade_descs: [
            'Increases firing\nspeed.',
            'Increases bullet\ndamage.',
            'Increases movement\nspeed.',
            'Increases pickup\nradius.',
            'Increases Max HP\nby 10.',
            'Enemies that hit you\ntake damage back.',
            'Bullets pierce 1 enemy,\n-50% dmg to the next.',
            'Blocks 10% of damage\nper level (max 30%).',
            '+10% critical hit\nchance.',
            'Orbiting sphere deals\n50% dmg, faster/level.',
            'Chance to fire twice:\n15% +10%/lvl (max 35%).',
        ],

        card_legendary: '★ LEGENDARY ★',
        card_common: 'COMMON',
        card_rare: '◆ RARE ◆',

        ability_names: ['INVINCIBILITY', 'GROUND SLAM', 'DISC STORM', 'LASER', 'SKULL', 'SONIC WAVE', 'SHATTER'],

        shop_skilltree: 'SKILL TREE',
        shop_artifacts: 'ARTIFACTS',
        shop_coins: 'Coins',
        shop_slots: 'SLOTS',
        shop_back: '[ ESC  -  Back ]',
        shop_max: 'MAX',
        shop_lv: 'Lv',
        shop_locked: 'LOCKED',
        shop_unequip: 'UNEQUIP',
        shop_active: 'ACTIVE',
        shop_equip: 'EQUIP',
        shop_owned: 'OWNED',
        shop_slots_full: 'SLOTS FULL',
        branch_names: ['ATTACK', 'SURVIVAL', 'MOBILITY'],
        node_titles: [
            ['DAMAGE', 'CRIT CHANCE', 'MULTISHOT'],
            ['MAX HP', 'REGEN', 'ARMOR'],
            ['SPEED', 'DASH', 'MAGNET'],
        ],
        node_descs: [
            ['Attack power +1', '+5% crit chance', 'Every 8th shot +1'],
            ['Max HP +10', 'Heal 10 HP over time', '-20% damage / lvl'],
            ['Move speed +10', 'Upgrade dash', 'Pickup radius +50'],
        ],

        artifact_names: ['BLOOD PACT', 'GLASS CANNON', 'ECHO CHAMBER', 'SOUL LEECH', 'BERSERKER', 'IRON SKIN', 'MAGNET CORE'],
        artifact_descs: [
            'Kill heals 2 HP',
            '+30% dmg, Max HP -20',
            'Bullets ricochet off walls',
            '+0.5% crit/kill  (max +5%)',
            'HP<=40%: dmg x1.5, no dash slow',
            'First 3 hits/run blocked',
            'Infinite pickup range',
        ],

        hud_lvl: 'LVL',
        hud_hp: 'HP',
        boss_name: 'THE OVERSEER',
        boss2_name: 'BASS DROP',
        boss3_name: 'STROBE',
        rhino_name: 'RHINO',
        boss_duet: 'DOCTOR + TELEPORTER',
        phase: 'PHASE',
        clear_all: 'CLEAR ALL ENEMIES',

        pause_stats: 'STATS',
        pause_build: 'BUILD',
        stat_damage: 'Damage',
        stat_firerate: 'Fire rate',
        stat_speed: 'Speed',
        stat_crit: 'Crit',
        stat_hp: 'Max HP',
        stat_armor: 'Armor',
        stat_magnet: 'Pickup',
        stat_dash: 'Dash',
        build_cards: 'Cards',
        build_abilities: 'Abilities',
        build_artifacts: 'Artifacts',
        build_none: '—',
        summary_title: 'RUN SUMMARY',
        summary_score: 'Score',
        summary_time: 'Time',
        summary_level: 'Level',
        summary_kills: 'Kills',
        summary_coins: 'Coins',
        gameover_title: 'GAME OVER',
        gameover_hint: 'R  -  Restart        Q  -  Hub',

        crazy_title: 'FRENZY',
        crazy_hint: 'ENEMIES ENRAGED — ESCAPE THROUGH THE PORTAL',
        hint_move: 'WASD — Move',
        hint_aim: 'Mouse — Aim (auto-fire)',
        hint_dash: 'Space — Dash',
        hint_ability: 'Q / E / R — Abilities',
        hint_pause: 'P — Pause',
        stageclear_title: 'STAGES CLEARED',
        stageclear_sub: 'You broke through all three stages',
        stageclear_stage: 'Stage',
        stageclear_total: 'TOTAL',
        stageclear_hub: 'GO TO HUB',

        set_cloud: 'Cloud Restore',
        cloud_title: 'CLOUD RESTORE',
        cloud_enter_nick: 'Enter your nick to restore progress:',
        cloud_warn: 'Backup is keyed by nick only (no password).',
        cloud_loading: 'Loading...',
        cloud_restored: 'Progress restored!',
        cloud_notfound: 'No backup found for this nick',
        cloud_offline: 'Cloud not available',
        cloud_hint: 'ENTER  -  Restore        ESC  -  Cancel',

        set_reset: 'Reset Character',
        reset_confirm: 'Click again to reset',
        reset_done: 'Character reset (name kept)',
    },

    ru: {
        menu_play: 'Играть',
        menu_records: 'Рекорды',
        menu_achievements: 'Достижения',
        menu_settings: 'Настройки',

        ach_title: 'ДОСТИЖЕНИЯ',
        ach_back: 'НАЗАД',
        ach_unlocked_run: 'Получены достижения!',
        ach_done: 'Выполнено',
        ach_of: 'из',
        ach_sort_default: 'По порядку',
        ach_sort_name: 'По названию',
        ach: {
            first_run:      { title: 'Первые шаги',      desc: 'Заверши первый забег' },
            rich_run:       { title: 'Скопидом',         desc: 'Собери 120 монет за забег' },
            ch1_clear:      { title: 'Глава 1 пройдена', desc: 'Победи финального босса главы 1' },
            ch2_clear:      { title: 'Глава 2 пройдена', desc: 'Победи финального босса главы 2' },
            ch3_clear:      { title: 'Гонтлет пройден',  desc: 'Одолей трио боссов главы 3' },
            score_10k:      { title: 'Меткий стрелок',   desc: 'Набери 10 000 очков за забег' },
            score_18k:      { title: 'Виртуоз',          desc: 'Набери 18 000 очков за забег' },
            survivor_10:    { title: 'Выживший',         desc: 'Продержись 10 минут за забег' },
            survivor_15:    { title: 'Последний герой',  desc: 'Продержись 15 минут за забег' },
            massacre_500:   { title: 'Бойня',            desc: 'Убей 500 врагов за забег' },
            massacre_1000:  { title: 'Неудержимый',      desc: 'Убей 1000 врагов за забег' },
            untouchable:    { title: 'Неприкосновенный', desc: 'Пройди главу, получив не больше 8 ударов' },
            hardcore_clear: { title: 'Герой хардкора',   desc: 'Пройди любую главу в режиме Hardcore' },
            slayer_1:       { title: 'Истребитель I',    desc: 'Убей 1 000 врагов суммарно' },
            slayer_2:       { title: 'Истребитель II',   desc: 'Убей 5 000 врагов суммарно' },
            slayer_3:       { title: 'Истребитель III',  desc: 'Убей 10 000 врагов суммарно' },
            slayer_4:       { title: 'Истребитель IV',   desc: 'Убей 25 000 врагов суммарно' },
            slayer_5:       { title: 'Истребитель V',    desc: 'Убей 50 000 врагов суммарно' },
            slayer_6:       { title: 'Истребитель VI',   desc: 'Убей 100 000 врагов суммарно' },
            veteran_1:      { title: 'Ветеран I',        desc: 'Сыграй 25 забегов' },
            veteran_2:      { title: 'Ветеран II',       desc: 'Сыграй 100 забегов' },
        },

        lobby_hub: 'ХАБ',
        lobby_start: 'НАЧАТЬ ЗАБЕГ',
        lobby_shop: 'УЛУЧШЕНИЯ И МАГАЗИН',
        lobby_back: 'Назад в меню',

        chapter_select_title: 'ВЫБОР ГЛАВЫ',
        chapter_label: 'ГЛАВА',
        chapter_play: 'ИГРАТЬ',
        chapter_locked: 'ЗАКРЫТО',
        chapter_hint_back: 'НАЗАД В ХАБ',

        set_title: 'НАСТРОЙКИ',
        set_grp_game: 'ИГРА',
        set_grp_audio: 'ЗВУК',
        set_grp_display: 'ЭКРАН',
        set_grp_account: 'АККАУНТ',
        set_cloud_open: 'открыть',
        set_hardcore: 'Хардкор',
        set_window: 'Режим окна',
        set_sound: 'Музыка',
        set_effects: 'Эффекты',
        set_language: 'Язык',
        set_rename: 'Сменить имя',
        on: 'ВКЛ',
        off: 'ВЫКЛ',
        win_full: 'ПОЛНЫЙ ЭКРАН',
        win_windowed: 'В ОКНЕ',
        not_set: '(не задано)',
        back: 'Назад',
        lang_en: 'English',
        lang_ru: 'Русский',
        cheat_gave: 'Получено 500 монет',
        cheat_noname: 'Имени пока нет — сначала установите рекорд',

        lb_title: 'РЕКОРДЫ',
        lb_hardcore: 'ХАРДКОР',
        lb_normal: 'ОБЫЧНЫЙ',
        lb_col_num: '#',
        lb_col_name: 'ИМЯ',
        lb_col_score: 'ОЧКИ',
        lb_col_time: 'ВРЕМЯ',
        lb_col_date: 'ДАТА',
        lb_hint_switch: '<  /  >   -   Обычный / Хардкор',
        lb_chapter: 'ГЛАВА',
        lb_your_place: 'Ваше место',
        lb_hint_chapter: '◀ ▶  Глава',
        lb_hint_mode: '▲ ▼  Режим',
        lb_hint_sort: 'TAB / клик  -  Сортировка (время / очки)',
        lb_hint_back: 'НАЗАД',

        pause_title: 'ПАУЗА',
        pause_resume: 'Продолжить',
        pause_restart: 'Заново',
        pause_quit: 'Выйти в хаб',

        gameover: 'ИГРА ОКОНЧЕНА\nНажми «R» — заново\nНажми «Q» — в хаб',
        gameover_records: 'L  -  Посмотреть рекорды',

        name_new_record: 'НОВЫЙ РЕКОРД!',
        name_score: 'Очки:',
        name_time: 'Время:',
        name_enter: 'Введите имя:',
        name_hint: 'ENTER  -  Подтвердить        BACKSPACE  -  Стереть',
        err_enter_name: 'Введите имя',
        err_name_taken: 'Имя занято',

        rename_title: 'СМЕНА ИМЕНИ',
        rename_current: 'Текущее имя:',
        rename_new: 'Новое имя:',
        rename_saving: 'Сохранение...',
        rename_merge_note: 'Объединяет рекорды, сохраняя лучшее время в каждом режиме.',
        rename_hint: 'ENTER  -  Подтвердить        ESC  -  Отмена',
        err_server: 'Ошибка сервера, попробуйте снова',

        levelup: 'НОВЫЙ УРОВЕНЬ!',
        first_card: 'СТАРТОВАЯ КАРТА',
        ability_choose: 'ВЫБЕРИТЕ СПОСОБНОСТЬ',
        cooldown: 'Перезарядка',
        ability_desc_0: 'Неуязвимость\nна 2 секунды.',
        ability_desc_1: 'Удар по земле:\nурон и отбрасывание\nближних врагов.',
        ability_desc_2: 'Выпустить 12 дисков\nво все стороны.',
        ability_desc_3: 'Выпустить пробивающий\nлуч в сторону курсора.',
        ability_desc_4: 'Запустить череп: рикошет\nпо 5 врагам, +20% урона\nза каждый отскок.',
        ability_desc_5: 'Звуковая волна: отбрасывает\nи бьёт всех врагов\nвокруг.',
        ability_desc_6: 'Бросает заряд, который\nвзрывается на 14 осколков\nво все стороны.',

        upgrade_toasts: ['УЛУЧШЕНИЕ: Скорострельность +', 'УЛУЧШЕНИЕ: Урон +', 'УЛУЧШЕНИЕ: Скорость +', 'УЛУЧШЕНИЕ: Магнит +', 'УЛУЧШЕНИЕ: Макс. HP +1', 'ОТКРЫТО: Блейдмейл', 'ОТКРЫТО: Прострел', 'УЛУЧШЕНИЕ: Энергощит', 'УЛУЧШЕНИЕ: Крит +10%', 'УЛУЧШЕНИЕ: Сфера', 'УЛУЧШЕНИЕ: Double Tap'],

        upgrade_titles: ['БЫСТРЫЙ КУРОК', 'ГРУБАЯ СИЛА', 'БЕГУН', 'МАГНЕТИЗМ', 'ПРИБАВКА HP', 'БЛЕЙДМЕЙЛ', 'ПРОСТРЕЛ', 'ЭНЕРГОЩИТ', 'УСИЛЕНИЕ КРИТА', 'СФЕРА', 'DOUBLE TAP'],
        upgrade_descs: [
            'Повышает скорость\nстрельбы.',
            'Повышает урон\nпуль.',
            'Повышает скорость\nпередвижения.',
            'Повышает радиус\nподбора.',
            'Повышает макс. HP\nна 10.',
            'Враг получает урон,\nкогда бьёт вас.',
            'Пуля пробивает врага,\n-50% урона следующему.',
            'Блокирует 10% урона\nза уровень (макс 30%).',
            '+10% к шансу\nкрит. удара.',
            'Орбит. сфера: 50% урона,\nбыстрее с уровнем.',
            'Шанс выстрелить дважды:\n15% +10%/ур (макс 35%).',
        ],

        card_legendary: '★ ЛЕГЕНДАРНАЯ ★',
        card_common: 'ОБЫЧНАЯ',
        card_rare: '◆ РЕДКАЯ ◆',

        ability_names: ['НЕУЯЗВИМОСТЬ', 'УДАР ОЗЕМЬ', 'ВИХРЬ ДИСКОВ', 'ЛАЗЕР', 'ЧЕРЕП', 'ЗВУКОВАЯ ВОЛНА', 'РАСКОЛ'],

        shop_skilltree: 'ДЕРЕВО НАВЫКОВ',
        shop_artifacts: 'АРТЕФАКТЫ',
        shop_coins: 'Монеты',
        shop_slots: 'СЛОТЫ',
        shop_back: '[ ESC  -  Назад ]',
        shop_max: 'МАКС',
        shop_lv: 'LVL',
        shop_locked: 'ЗАКРЫТО',
        shop_unequip: 'СНЯТЬ',
        shop_active: 'АКТИВЕН',
        shop_equip: 'НАДЕТЬ',
        shop_owned: 'КУПЛЕНО',
        shop_slots_full: 'НЕТ СЛОТОВ',
        branch_names: ['АТАКА', 'ВЫЖИВАНИЕ', 'МОБИЛЬНОСТЬ'],
        node_titles: [
            ['УРОН', 'ШАНС КРИТА', 'МУЛЬТИВЫСТРЕЛ'],
            ['МАКС. HP', 'РЕГЕН', 'БРОНЯ'],
            ['СКОРОСТЬ', 'РЫВОК', 'МАГНИТ'],
        ],
        node_descs: [
            ['Сила атаки +1', '+5% к шансу крита', 'Каждый 8-й выстрел +1'],
            ['Макс. HP +10', 'Лечит 10 HP со временем', '-20% урона / LVL'],
            ['Скорость +10', 'Улучшить рывок', 'Радиус подбора +50'],
        ],

        artifact_names: ['КРОВАВЫЙ ПАКТ', 'СТЕКЛЯННАЯ ПУШКА', 'ЭХО-КАМЕРА', 'ПОЖИРАТЕЛЬ ДУШ', 'БЕРСЕРК', 'ЖЕЛЕЗНАЯ КОЖА', 'МАГНИТНОЕ ЯДРО'],
        artifact_descs: [
            'Убийство лечит 2 HP',
            '+30% урона, макс. HP -20',
            'Пули рикошетят от стен',
            '+0.5% крита/убийство (макс +5%)',
            'HP<=40%: урон x1.5, рывок без замедл.',
            'Первые 3 удара за забег блокируются',
            'Бесконечный радиус подбора',
        ],

        hud_lvl: 'LVL',
        hud_hp: 'HP',
        boss_name: 'НАДЗИРАТЕЛЬ',
        boss2_name: 'БАС-ДРОП',
        boss3_name: 'СТРОБ',
        rhino_name: 'НОСОРОГ',
        boss_duet: 'ВРАЧ + ТЕЛЕПОРТЁР',
        phase: 'ЭТАП',
        clear_all: 'УНИЧТОЖЬТЕ ВСЕХ ВРАГОВ',

        pause_stats: 'СТАТЫ',
        pause_build: 'БИЛД',
        stat_damage: 'Урон',
        stat_firerate: 'Скорострел.',
        stat_speed: 'Скорость',
        stat_crit: 'Крит',
        stat_hp: 'Макс. HP',
        stat_armor: 'Броня',
        stat_magnet: 'Подбор',
        stat_dash: 'Рывок',
        build_cards: 'Карты',
        build_abilities: 'Способности',
        build_artifacts: 'Артефакты',
        build_none: '—',
        summary_title: 'ИТОГИ ЗАБЕГА',
        summary_score: 'Очки',
        summary_time: 'Время',
        summary_level: 'Уровень',
        summary_kills: 'Убито',
        summary_coins: 'Монеты',
        gameover_title: 'ИГРА ОКОНЧЕНА',
        gameover_hint: 'R  -  Заново        Q  -  В хаб',

        crazy_title: 'БЕЗУМИЕ',
        crazy_hint: 'ВРАГИ В ЯРОСТИ — УХОДИ В ПОРТАЛ',
        hint_move: 'WASD — Движение',
        hint_aim: 'Мышь — Прицел (автоогонь)',
        hint_dash: 'Пробел — Рывок',
        hint_ability: 'Q / E / R — Способности',
        hint_pause: 'P — Пауза',
        stageclear_title: 'ЭТАПЫ ПРОЙДЕНЫ',
        stageclear_sub: 'Ты прорвался через все три этапа',
        stageclear_stage: 'Этап',
        stageclear_total: 'ИТОГО',
        stageclear_hub: 'ПЕРЕЙТИ В ХАБ',

        set_cloud: 'Восстановить из облака',
        cloud_title: 'ВОССТАНОВЛЕНИЕ ИЗ ОБЛАКА',
        cloud_enter_nick: 'Введите ник для восстановления прогресса:',
        cloud_warn: 'Бэкап привязан только к нику (без пароля).',
        cloud_loading: 'Загрузка...',
        cloud_restored: 'Прогресс восстановлён!',
        cloud_notfound: 'Бэкап для этого ника не найден',
        cloud_offline: 'Облако недоступно',
        cloud_hint: 'ENTER  -  Восстановить        ESC  -  Отмена',

        set_reset: 'Сбросить персонажа',
        reset_confirm: 'Нажми ещё раз для сброса',
        reset_done: 'Персонаж сброшен (имя сохранено)',
    },
};

let I18N_LANG = 'en';

function normLang(code) { return I18N[code] ? code : 'en'; }

function detectLang() {
    try {
        const l = (navigator.language || navigator.userLanguage || '').toLowerCase();
        if (l.indexOf('ru') === 0) return 'ru';
    } catch (e) {}
    return 'en';
}

function setLanguage(code) { I18N_LANG = normLang(code); }

function t(key) {
    const cur = I18N[I18N_LANG] || I18N.en;
    let v = cur[key];
    if (v === undefined) v = I18N.en[key];
    return v === undefined ? key : v;
}
